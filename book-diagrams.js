...
function simultaneous(v=0,nonlinear=false){
 const k=v%6;
 if(nonlinear){
   if(k===0||k===3){let b=axes();let pts=[];for(let x=-4;x<=4;x+=.15){let y=.35*x*x-2;pts.push((120+x*55)+","+(275-y*24));}b+='<polyline points="'+pts.join(" ")+'" fill="none" stroke="'+C.blue+'" stroke-width="4"/>'+L(110,250,565,85,'stroke="'+C.red+'" stroke-width="4"');return svg(title("LINE + CURVE INTERSECTIONS")+b);}
   if(k===1||k===4)return svg(title("CIRCLE + LINE")+O(320,180,115,'fill="'+C.pale+'" stroke="'+C.blue+'" stroke-width="4"')+L(120,280,535,65,'stroke="'+C.red+'" stroke-width="4"')+O(230,222,7,'fill="'+C.ink+'"')+O(407,126,7,'fill="'+C.ink+'"')+T(150,325,"0, 1 or 2 intersection points = number of solutions",sm));
   return svg(title("SUBSTITUTION INTO NON-LINEAR EQUATION")+R(65,85,215,150,'rx="18" fill="'+C.pale+'" stroke="'+C.ink+'" stroke-width="3"')+R(360,85,215,150,'rx="18" fill="#fff8de" stroke="'+C.ink+'" stroke-width="3"')+T(105,140,"y = ...",lab)+T(400,140,"x²+y²=...",lab)+L(280,160,360,160,'stroke="'+C.blue+'" stroke-width="4"'));
 }
